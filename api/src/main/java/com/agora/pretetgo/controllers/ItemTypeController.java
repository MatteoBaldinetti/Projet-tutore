...
public class ItemTypeController {
    ...
    @PatchMapping("/{id}")
    public ResponseEntity<ItemTypeResponseDTO> patchItemType(@RequestBody ItemTypeInsertDTO dto, @PathVariable Long id) {
        return ResponseEntity.ok(
                itemTypeMapper.toResponseDTO(itemTypeService.patchItemType(id, dto))
        );
    }
    ...
    @ApiResponse(responseCode = "200", description = "List of itemTypes retrieved successfully")
    @GetMapping("/search")
    public ResponseEntity<List<ItemTypeResponseDTO>> searchItemTypes(@ParameterObject @ModelAttribute ItemTypeFilterDTO filterDTO) {
        return ResponseEntity.ok(
