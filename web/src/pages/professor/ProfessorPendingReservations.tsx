...
export default function ProfessorPendingReservations() {
    ...
    const fetchAll = async () => {
        try {
            const [resourcesRes, reservationsRes, groupsRes, gsRes, studentsRes] = await Promise.all([
            ...
            const pending: Reservation[] = reservationsRes.data.filter(
                (r: Reservation) => myIds.has(r.ressourceId) && r.status === "PENDING"
                ...
                <div className="bg-white rounded-xl shadow-md p-6 mb-6 flex items-center justify-between">
                    <div>
                        ...
                        <p className="text-gray-500 mt-1">
                            {loading ? "…" : `${pendingReservations.length} demande${pendingReservations.length !== 1 ? "s" : ""} en attente`}
                        </p>
                    </div>
                    {!loading && pendingReservations.length > 0 && (
                        <div className="w-12 h-12 rounded-full bg-yellow-100 border-2 border-yellow-300 flex items-center justify-center">
                            <span className="text-yellow-700 font-bold text-lg">{pendingReservations.length}</span>
                        </div>
                    )}
                </div>
                ...
                {loading ? (
                    ...
                    <div className="bg-white rounded-xl shadow-md p-16 text-center">
                        <div className="flex justify-center mb-4">
                            <svg width="56" height="56" viewBox="0 0 24 24" fill="none" className="text-green-400">
                                <path d="M20 6L9 17L4 12" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/>
                            </svg>
                        </div>
                        <p className="text-xl font-semibold text-gray-600 mb-2">Tout est à jour !</p>
                        <p className="text-gray-400 italic text-sm">Aucune réservation en attente de validation.</p>
                    </div>
                ) : (
                    <div className="space-y-4">
                        {pendingReservations.map(r => (
                            <div key={r.id} className="bg-white rounded-xl shadow-md p-6 border-l-4 border-yellow-400">
                                <div className="flex flex-col lg:flex-row lg:items-start justify-between gap-4">

                                    {/* INFOS PRINCIPALES */}
                                    <div className="flex-1 grid grid-cols-1 md:grid-cols-2 gap-4">
                                        <div>
                                            <p className="text-xs text-gray-400 uppercase font-medium mb-1">Ressource</p>
                                            <p className="font-semibold text-gray-800">{getResourceName(r.ressourceId)}</p>
                                        </div>
                                        <div>
                                            <p className="text-xs text-gray-400 uppercase font-medium mb-1">Groupe demandeur</p>
                                            <p className="font-medium text-gray-700">{getGroupName(r.reserveById)}</p>
                                        </div>
                                        <div>
                                            <p className="text-xs text-gray-400 uppercase font-medium mb-1">Étudiants</p>
                                            <p className="text-sm text-gray-600">{getGroupStudentNames(r.reserveById)}</p>
                                        </div>
                                        <div>
                                            <p className="text-xs text-gray-400 uppercase font-medium mb-1">Demandé le</p>
                                            <p className="text-sm text-gray-600">{formatDate(r.createdAt)}</p>
                                        </div>
                                        <div className="md:col-span-2">
                                            <p className="text-xs text-gray-400 uppercase font-medium mb-1">Créneau demandé</p>
                                            <p className="text-sm text-gray-700 font-medium">
                                                Du <span className="text-[#3A8C85]">{formatDate(r.startDate)}</span> au <span className="text-[#3A8C85]">{formatDate(r.endDate)}</span>
                                                <span className="ml-2 text-xs text-gray-400">({formatDuration(r.startDate, r.endDate)})</span>
                                            </p>
                                        </div>
                                    </div>
