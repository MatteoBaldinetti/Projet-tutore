...
export default function AdminProfile() {
    ...
    useEffect(() => {
        const fetchAdmin = async () => {
            if (!userId) return;
            try {
                const res = await axios.get(`${API_URL}/admins/${userId}`, {
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            await axios.put(`${API_URL}/admins/${userId}`, payload, {
                headers: { "x-api-key": API_KEY, "Content-Type": "application/json" },
            });
            setAdmin({ ...admin, firstName, lastName, email });
            updateContext(userId, email, firstName, lastName, "ADMIN");
            ...
            <div className="min-h-screen bg-gray-100 p-6">
                <div className="w-full bg-white rounded-xl shadow-md p-8">
                    ...
                    <div className="flex items-center gap-6 mb-8">
                        <div className="admin-avatar w-20 h-20 rounded-full flex items-center justify-center text-white text-2xl font-bold shrink-0 select-none">
                            {initials}
                        </div>
                        <div>
                            <h1 className="text-2xl font-bold text-gray-800">{admin?.firstName} {admin?.lastName}</h1>
                            <p className="text-gray-500 text-sm">{admin?.email}</p>
                            <p className="text-xs text-[#3A8C85] font-medium mt-1">Administrateur</p>
                        </div>
                        {!isEditing && (
                            <button
                                onClick={() => setIsEditing(true)}
                                className="ml-auto edit-admin-btn text-white px-5 py-2 rounded flex items-center gap-2 transition cursor-pointer"
                            >
                                <IconEdit />
                                Modifier
                            </button>
                    ...
                    {isEditing ? (
                        <div>
                            <h2 className="text-lg font-semibold mb-4 admin-section-title">Modifier mes informations</h2>
                            ...
                            <div className="flex justify-end gap-3">
                                <button onClick={handleCancel} className="px-4 py-2 cancel-edit-admin-btn rounded transition cursor-pointer">Annuler</button>
                                <button onClick={handleSave} className="px-4 py-2 save-admin-btn text-white rounded transition cursor-pointer">Enregistrer</button>
                            </div>
                        </div>
                    ) : (
                        <div>
                            <h2 className="text-lg font-semibold mb-4 admin-section-title">Mes informations</h2>
                            {successMessage && <p className="text-green-600 text-sm mb-4">{successMessage}</p>}
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                {[
                                    { label: "Prénom",        value: admin?.firstName },
                                    { label: "Nom",           value: admin?.lastName },
                                    { label: "Email",         value: admin?.email },
                                    { label: "Compte actif",  value: admin?.enabled ? "Oui" : "Non" },
                                    { label: "Membre depuis", value: admin?.createdAt ? new Date(admin.createdAt).toLocaleDateString("fr-FR") : "—" },
