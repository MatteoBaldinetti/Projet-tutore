...
import type { Classroom, Professor } from "../../../types/types";
import "../../styles/ManageClassrooms.css";
...
export default function ManageClassrooms() {
    ...
    const [showAddClassroom, setShowAddClassroom] = useState<boolean>(false);
    ...
    const [classroomTypeId, setClassroomTypeId] = useState<number | "">("");
    ...
    const filteredClassrooms = classroomList.filter((c) => {
        const value = search.toLowerCase();
        return c.name.toLowerCase().includes(value) || c.description.toLowerCase().includes(value);
    });
    ...
    const handleAddClassroomButtonPress = () => {
        setEditingClassroom(null);
        resetForm();
        setShowAddClassroom(true);
    };

    const handleEditClassroomButtonPress = (classroom: Classroom) => {
    ...
    const addOrUpdateClassroom = async () => {
        try {
            const payload = {
                ...
                available,
                classroomTypeId: classroomTypeId || null,
                managedByIds,
                createdAt: editingClassroom ? editingClassroom.createdAt : new Date().toISOString(),
            };

            if (editingClassroom) {
                await axios.put(`${API_URL}/classrooms/${editingClassroom.id}`, payload, {
            ...
            console.error("Erreur lors de l'enregistrement de la salle :", err);
        }
    };
    ...
    const getClassroomTypeName = (id: number) => {
        return classroomTypeList.find((t) => t.id === id)?.name || "—";
    };

    useEffect(() => {
        if (showAddClassroom || classroomToDelete !== null) {
            document.body.style.overflow = "hidden";
        } else {
            document.body.style.overflow = "";
        }
        return () => { document.body.style.overflow = ""; };
    }, [showAddClassroom, classroomToDelete]);

    return (
        <Layout
            titleHeader="Gestion des salles"
            children={
                    ...
                    <div className="w-full mx-auto bg-white rounded-xl shadow-md p-6">
                        ...
                        <div className="flex justify-between items-center mb-6 gap-4">
                            ...
                            <div className="flex items-center gap-3">
                                ...
                                />
                                <button onClick={handleAddClassroomButtonPress} className="add-classroom-btn text-white px-4 py-2 rounded transition cursor-pointer whitespace-nowrap">
                                    + Ajouter une salle
                                </button>
                            </div>
                        </div>
                            ...
                            <table className="w-full text-md">
                                <thead className="bg-[#E8F4F3] border border-[#E8F4F3]">
                                    <tr>
                                        ...
                                        <th className="text-[#3A8C85] px-4 py-4 font-normal uppercase text-left w-1/4">N° de salle</th>
                                ...
                                <tbody>
                                        ...
                                        <tr key={classroom.id}>
                                            ...
                                            <td className="px-4 py-3 text-left border-b border-[#F1F5F9]">
                                                <span className={`px-2 py-1 rounded-full text-xs font-medium ${classroom.available ? "bg-green-100 text-green-700" : "bg-red-100 text-red-700"}`}>
                                                    {classroom.available ? "Oui" : "Non"}
                                                </span>
                                            </td>
                                            <td className="px-4 py-3 text-center space-x-4 border-b border-[#F1F5F9]">
                                                <button onClick={() => handleEditClassroomButtonPress(classroom)} className="cursor-pointer">
                                                ...
                                                <button onClick={() => setClassroomToDelete(classroom.id)} className="cursor-pointer">
                                    ...
                                    {filteredClassrooms.length === 0 && (
                                        <tr>
                                            <td colSpan={5} className="text-center py-6 text-gray-500 italic">Aucune salle trouvée</td>
                                        </tr>
                                    )}
                                </tbody>
                            </table>
                        ...
                        {showAddClassroom && (
                            <div className="fixed inset-0 bg-black/50 flex items-center justify-center">
                                <div className="bg-white p-6 rounded-xl w-250 max-h-[90vh] overflow-y-auto">
                                    <h2 className="text-xl font-semibold mb-4">{editingClassroom ? "Modifier la salle" : "Ajouter une salle"}</h2>
                                    <form onSubmit={(e) => { e.preventDefault(); addOrUpdateClassroom(); }}>
                                        <div className="grid grid-cols-2 gap-4 mb-3">
                                            ...
                                            <div className="flex flex-col">
                                                <label htmlFor="roomNumber" className="mb-1 font-medium">N° de salle</label>
                                                <input id="roomNumber" type="number" min="0" value={roomNumber} onChange={(e) => setRoomNumber(e.target.value)} className="border border-gray-300 rounded px-3 py-2 focus:outline-none focus:ring-2 focus:ring-[#3A8C85]" />
                                            </div>
                                        </div>
                                        <div className="grid grid-cols-1 gap-4">
                                            ...
                                            <div className="flex flex-col">
                                                <label htmlFor="classroomType" className="mb-1 font-medium">Type de salle</label>
                                                <select id="classroomType" value={classroomTypeId} onChange={(e) => setClassroomTypeId(Number(e.target.value))} className="border border-gray-300 rounded px-3 py-2 focus:outline-none focus:ring-2 focus:ring-[#3A8C85]">
                                                    <option value="">— Sélectionner un type —</option>
                                                    {classroomTypeList.map((t) => (
                                        ...
                                        <div className="flex justify-end gap-2 mt-4">
                                            <button type="button" onClick={() => { setShowAddClassroom(false); setEditingClassroom(null); resetForm(); }} className="px-4 py-2 cancel-send-classroom-btn rounded transition cursor-pointer">Annuler</button>
                                            <button type="submit" className="px-4 py-2 send-classroom-btn text-white rounded transition cursor-pointer">Enregistrer</button>
                                        </div>
                                    </form>
                                </div>
                            </div>
                            ...
                            <div className="fixed inset-0 bg-black/50 flex items-center justify-center">
                                <div className="bg-white p-6 rounded-xl w-150 shadow-lg">
                                    ...
                                    <div className="flex justify-end gap-4 mt-6">
                                        <button onClick={() => setClassroomToDelete(null)} className="px-4 py-2 rounded cancel-delete-classroom-btn transition cursor-pointer">Annuler</button>
                                        <button onClick={() => classroomToDelete && deleteClassroom(classroomToDelete)} className="px-4 py-2 bg-red-600 hover:bg-red-700 text-white rounded transition cursor-pointer">Supprimer</button>
                                    </div>
                                </div>
                            </div>
                        )}
